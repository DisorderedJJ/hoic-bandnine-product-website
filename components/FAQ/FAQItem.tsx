    ...
    <>
      <div className="flex flex-col border-b border-stroke last-of-type:border-none dark:border-strokedark">
        ...
        {ans.length === 1 ? (
          ...
          <ol
            className={`relative ml-3 border-s border-gray-200 text-gray-500 dark:border-gray-700 dark:text-gray-400 ${
              activeFaq === id ? "block" : "hidden"
            }`}
          >
            {ans.map((current, index) => {
              return (
                <p
                key={index}
                  className={`border-t dark:border-strokedark lg:px-9 lg:py-3`}
                >
                  {current}
                </p>
              );
